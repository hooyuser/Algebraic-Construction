...
\dfn{Opposite Group}{
    ...
    \[
        \mathsf{B}G^{\mathrm{op}}\simeq (\mathsf{B}G)^{\mathrm{op}}.
    \]
}
...
\[
    G\text{-}\mathsf{Set}\simeq\mathrm{Fun}(\mathsf{B}G, \mathsf{Set})    
\]
...
\dfn{Centralizer}{
    ...
    \[
      \mathrm{C}_G(x)=\{ g\in G\mid gxg^{-1}=x   \}.
    \]
}
...
\dfn{Normal Subgroup}{
    Let $G$ be a group. A subgroup $H$ of $G$ is called a \textbf{normal subgroup} if $gHg^{-1}=H$ for all $g\in G$.
}
...
\dfn{Outer Automorphism Group}{
    Let $G$ be a group. Then we have $\mathrm{Inn}(G) \triangleleft \mathrm{Aut}(G)$. And the \textbf{outer automorphism group} of $G$ is defined as 
    $$
...
\dfn{Commutator}{
    Let $G$ be a group. Define a binary operation defined on $G$ as
    \begin{align*}
