...
\begin{tabular}{|P{4.4cm}|m{5.3cm}|m{5.3cm}|}
    ...
    \begin{center}
        \begin{tikzcd}[every arrow/.append style={-latex, line width=1.2pt}]
        \circ  \arrow[rrd, draw=cyan, bend left] \arrow[rdd, draw=cyan, bend right] &[-2em]                                 &  [-0.4em]                  \\[-0.3cm]
                                                                                & \varinjlim\arrow[lu, dash pattern=on 4pt off 2pt, draw=arrowRed]  & \bullet   \arrow[l, draw=arrowBlue]\\
                                                                                & \bullet \arrow[u, draw=arrowBlue]             & \bullet  \arrow[l] \arrow[u]      
        \end{tikzcd} 
    \end{center} \\
    \hline
    $\begin{tikzcd}[every arrow/.append style={-latex, line width=1.2pt}]
...
\prop{Evaluation Functor Preserves Limits}{
...
\[
    G\text{-}\mathsf{Set}\cong\mathrm{Fun}(\mathsf{B}G, \mathsf{Set})    
\]
...
\dfn{Product of $G$-Sets}{
    ...
    \]
    Alternatively, the product of two $G$-sets can be defined as the product of two functors.
}
\dfn{Coproduct of $G$-Sets}{
    ...
    \end{cases} 
    \]
    Alternatively, the coproduct of two $G$-sets can be defined as the coproduct of two functors.
}
...
\ex{Acting on Orbit}{
    Let $G$ be a group acting on a set $X$ and $x\in X$. Then $G$ acts on $Gx$ by left multiplication.
}
\dfn{Orbit Space}{
...
\prop{Orbit Decomposition}{
    ...
    x\sim y \iff Gx=Gy.
    \]
    Then the equivalence class of $x$ is exactly $Gx$. And we have a partition of $X$ by the orbits of $G$ acting on $X$
    \[
    X=\bigsqcup_{x\in X}Gx=X/G    .
    \]
}
\proof{
...
\prop{$G$-set Isomorphism $G/\mathrm{Stab}_G(x)^L\cong Gx$}{
