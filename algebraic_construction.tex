...
\dfn{Limit}{
    ...
    \[
            ...
            \end{tikzcd}
    \]
    where $C\in \mathrm{Ob}(\mathsf{C})$. Such $h:\Delta C\Rightarrow F$ is called a \textbf{cone} with vertex $C$ over $F$ because its commutative square simplifies to the following communitative triangle
    \[
        \begin{tikzcd}[ampersand replacement=\&]
...
\]    
    ...
    \[
        \begin{tikzcd}[ampersand replacement=\&, row sep=9pt]
            ...
            \end{tikzcd}
    \]
    The \textbf{limit} of $F$, denoted by $\varprojlim F$, is the terminal object of $(\Delta\downarrow \Delta F)$. 
    \[
        \begin{tikzcd}[ampersand replacement=\&, every arrow/.append style={-latex, line width=1.1pt}]
            ...
            \&                   \& \varprojlim F\arrow[ld, draw=arrowBlue, shorten <=-3.5pt, "\phi_i" yshift=4.5pt] \arrow[rd, draw=arrowBlue, shorten <=-4.5pt, "\phi_j"' yshift=4.5pt]               \&                   \\[-0.1cm]
        \& F(i) \arrow[rr, "F(\lambda)"']\&   \& F(j)
        \end{tikzcd}
    \]
}
...
\noindent Topological groups are the group objects in the category $\mathsf{Top}$ of topological spaces.\\
