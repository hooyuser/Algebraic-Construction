...
\prop{Properties of Commutator}{
    ...
    \begin{enumerate}[label=(\roman*)]
        \item $x$ commutes with $y$ if and only if $[x,y]=1$.
        \item $[x,y]^{-1}=[y,x]$.
        \item For any homomorphism $f:G\longrightarrow H$, $f([x,y])=[f(x),f(y)]$.
    \end{enumerate}
}

\dfn{Left Multiplication Action}{
    Let $G$ be a group. The \textbf{left multiplication action} of $G$ on itself is defined as 
    \begin{align*}
        G &\longrightarrow \mathrm{Aut}(G)\\
...
\dfn{Word Map}{
    Let $G$ be a group. A \textbf{word map} is a map $w:G^n\longrightarrow G$ defined by 
    $$
    w(x_1,\cdots,x_n)=x_1^{\alpha_1}\cdots x_n^{\alpha_n},
    $$ 
    where $\alpha_i\in\mathbb{Z}$ for all $i$.
}
\dfn{Verbal Subgroup}{
