...
\dfn{Valuation of Field}{
    ...
    \begin{itemize}
        \item a field $K$ and its multiplicative group $K^{\times}$,
        \item an abelian totally ordered group $(\Gamma,+, \geq)$.
    \end{itemize}
The ordering and group law on $\Gamma$ are extended to the set $\Gamma \cup\{\infty\}$ by the rules:
\begin{itemize}
    \item $\infty \geq \alpha$ for all $\alpha \in \Gamma$,
    \item $\infty+\alpha=\alpha+\infty=\infty+\infty=\infty$ for all $\alpha \in \Gamma$.
\end{itemize}
...
A valuation $v$ is trivial if $v(a)=0$ for all $a$ in $K^{\times}$, otherwise it is non-trivial.
