...
\dfn{Normalizer}{
    Let $G$ be a group and $E$ be a subset of $G$. The \textbf{normalizer} of $E$ in $G$ is defined as 
    $$
    \mathrm{N}_G(E)=\left\{ g\in G\mid gEg^{-1}=E \right\}.
    $$ 
    Let $G$ acts on $2^G$ by conjugation. Then $\mathrm{N}_G(E)=\mathrm{Stab}_G(E)$.
}
\prop{Properties of Normalizer}{
    Let $G$ be a group and $H$ be a subset of $G$. Then
    \begin{enumerate}[(i)]
        \item $\mathrm{N}_G(H)$ is a subgroup of $G$.
        \item If $H\le G$, then $H\lhd\mathrm{N}_G(H)$. Moreover, $\mathrm{N}_G(H)$ is the largest subgroup of $G$ in which $H$ is normal, i.e. $$H\lhd K\le G\implies K\le \mathrm{N}_G(H).$$
    \end{enumerate}
}
\pf{
...
\ex{$\mathrm{Aut}_\mathsf{C}(X)$ acts on $\mathrm{Hom}_\mathsf{C}(X,Y)$ and $\mathrm{Hom}_\mathsf{C}(Y,X)$}{
...
\pf{
    ...
    \begin{itemize}
        ...
        \[
        \bar{m}=\bar{n}\in\mathrm{N}_G(H)/H\implies m\in nH\implies \exists h\in H,m=nh\implies\phi(\bar{m})(s)=gm\cdot s=gnh\cdot s=gn\cdot s=\phi(\bar{n})(s).
        \]
        \item $\phi(\bar{n})$ is a $G$-set morphism: $\phi(\bar{n})(g\cdot s)=gn\cdot s=g\cdot\phi(\bar{n})(s)$
        \item $\phi(\bar{n})$ is a bijection: $\phi(\bar{n}^{-1})$ is the inverse of $\phi(\bar{n})$.
    \end{itemize}
    For any automorphism $\psi\in \mathrm{Aut}_{G\text{-}\mathsf{Set}}(S)$, by transitivity we have $\psi(s)=n\cdot s$ for some $n \in G$. For $h \in H$, $hn\cdot s=h\cdot\psi(s)=\psi(h\cdot s)=\psi(s)=n\cdot s$, hence $n^{-1} h n \in H$ and $n \in N H$. This gives a well-defined map 
    \begin{align*}
