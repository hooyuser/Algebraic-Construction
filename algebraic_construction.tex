...
\dfn{Enriched Category}{
    ...
    \begin{enumerate}[(i)]
        ...
        \]
        \item Identity: for each object $A\in \mathrm{Ob}(\mathsf{C})$, a morphism $\mathrm{id}_A:1 \to \mathrm{Hom}_{\mathsf{C}}(A,A)$ in $\mathsf{V}$.
    \end{enumerate}
    such that the following conditions hold
    \begin{enumerate}[(i)]
        \item For each quadruple of objects $A,B,C,D\in \mathrm{Ob}(\mathsf{C})$, the following diagram commutes
        \[
            \begin{tikzcd}[ampersand replacement=\&, column sep=small]
