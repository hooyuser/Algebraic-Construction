...
\dfn{Normalizer}{
    Let $G$ be a group and $H$ be a subgroup of $G$. The \textbf{normalizer} of $H$ in $G$ is defined as 
    $$
    \mathrm{N}_G(H)=\left\{ g\in G\mid gHg^{-1}=H \right\}.
    $$ 
}
\prop{Properties of Normalizer}{
    Let $G$ be a group and $H$ be a subset of $G$. Then
    \begin{enumerate}[(i)]
        \item $\mathrm{N}_G(H)$ is a subgroup of $G$.
        \item If $H\le G$, then $\mathrm{N}_G(H)$ is the largest subgroup of $G$ in which $H$ is normal. That is, if $H\lhd K\le G$, then $K\le \mathrm{N}_G(H)$.
    \end{enumerate}
}
\pf{
    (i) is trivial. For (ii), let $H\lhd K\le G$. Then $\forall k\in K$, $kHk^{-1}=H$. Thus $k\in \mathrm{N}_G(H)$.
}
\dfn{Centralizer}{
...
\prop{Properties of Centralizer}{
    Let $G$ be a group and $H$ be a subset of $G$. Then
    \begin{enumerate}[(i)]
        \item $\mathrm{C}_G(H)\lhd \mathrm{N}_G(H)\le G$.
    \end{enumerate}
}
...
\thm{N/C Theorem}{
    Let $G$ be a group and $H$ be a subgroup of $G$. Define
    \begin{align*}
        \Psi: \mathrm{N}_G(H) &\longrightarrow \mathrm{Aut}(H)\\
        g &\longmapsto \gamma_g|_H
    \end{align*}
    Then we have $\ker\Psi=\mathrm{C}_G(H)$ and $\mathrm{N}_G(H)/\mathrm{C}_G(H) \cong \mathrm{Im}\Psi\le \mathrm{Aut}(H)$.
}
...
\ex{Acting on Power Set}{
    If a group $G$ acts on a set $X$, then $G$ acts on the power set $\mathcal{P}(X)$ by
    \[
...
\ex[acting_on_functions]{Acting on Functions}{
    If $G$ acts on $X$, then it also acts on $Y^X$ for any set $Y$. The action on $Y^X$ is given as follows: for all $g\in G$, $f\in Y^X$ and $x\in X$,
    \[
...
\dfn{Conjugacy Action and Inner Automorphism Group}{
    Let $G$ be a group. The \textbf{conjugacy action} of $G$ on itself is defined as 
    \begin{align*}
        \gamma:G &\longrightarrow \mathrm{Aut}(G)\\
...
\prop{Equivalent Characterization of Inner Automorphisms}{
...
\dfn{$k$-Cycle}{
    ...
    \begin{enumerate}[(i)]
        \item $\sigma(a_i)=a_{i+1}\quad\text{for }i=1,2,\cdots,k-1$,
        \item $\sigma(a_k)=a_1$,
        \item $\sigma(x)=x\quad\text{for }x\in P_n-A$.
    \end{enumerate}
}

\thm{Pólya Enumeration Theorem (Unweighted)}{
    Let $X, Y$ be finite sets, where $X=\{1,2,\cdots,n\}$ is the set of points to be colored and $Y$ is the set of colors. Suppose a group $G$ acts on $X$ through $\sigma:G\to\mathrm{Aut}_{\mathsf{Set}}(X)$. Then it also acts on $Y^X$ by \Cref{th:acting_on_functions}. Define that a coloring configuration of $(X,Y,\sigma)$ is an orbit in the $G$-set $Y^X$. Then the number of distinct coloring configurations is
$$
\left|Y^X / G\right|=\frac{1}{|G|} \sum_{g \in G}|Y|^{c(g)},
$$
where $c(g)$ denotes the number of cycles in the cycle decomposition of $\sigma_g \in \mathrm{Aut}_{\mathsf{Set}}(X)$.
}
\pf{ 
    We apply Burnside's Lemma, which states that
$$
\left|Y^X / G\right|=\frac{1}{|G|} \sum_{g \in G}\left|\left(Y^X\right)^g\right| .
$$
It remains to show that $\left|\left(Y^X\right)^g\right|=|Y|^{c(g)}$. But any function $f \in Y^X$ will remain constant under the action of $g$ if and only if all elements in $X$ in each cycle are assigned the same set element in $Y$. There are thus $|Y|$ choices of elements in $Y$ for each of the $c(g)$ cycles in the cycle decomposition, and the result follows.
}
