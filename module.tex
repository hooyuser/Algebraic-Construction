...
$R$-$\mathsf{Mod}$ is an abelian category, so it is $\mathsf{Ab}$-enriched. When $R$ is a commutative ring, $R$-$\mathsf{Mod}$ is even self-enriched.
...
\begin{definition}{Determinant of $R$-linear Transformation on Free Module of Finite Rank}{}
    ...
    \begin{enumerate}[(i)]
        \item The \hyperref[ex:take_degree_k_functor]{functor $\Largewedge^n:R\text{-}\mathsf{Mod}\to R\text{-}\mathsf{Mod}$} induces a map between hom-sets 
        \begin{align*}
        ...
        \begin{align*}
            \mathrm{coor}_{\mathrm{id}_{\largewedge^n(M)}}:\mathrm{End}_{R\text{-}\mathsf{Mod}}\left(\largewedge^n(M)\right)&\xlongrightarrow{\sim} R\\
            \mathrm{id}_{\largewedge^n(M)} &\longmapsto 1_R.
        \end{align*}
        The \textbf{determinant} map is defined as the composition
         \begin{align*}
        \det:=\mathrm{coor}_{\mathrm{id}_{\largewedge^n(M)}}\circ \largewedge^n: \mathrm{End}_{R\text{-}\mathsf{Mod}}(M)&\longrightarrow R\\
        f &\longmapsto \det(f):= \mathrm{coor}_{\mathrm{id}_{\largewedge^n(M)}}\left(f\wedge\cdots\wedge f\right).
        \end{align*}
        $\det(f)$ is called the determinant of $f$.
        \item Suppose $\{e_1,\cdots,e_n\}$ is a baisis of $M$, then for any $f\in \mathrm{End}_{R\text{-}\mathsf{Mod}}\left(M\right)$, it can be uniquely represented by a matrix $A=(a_{ij})$ with respect to the basis $\{e_1,\cdots,e_n\}$
        \[
        (f(e_1), \cdots, f(e_n))=(e_1, \cdots, e_n)A.
        \]
        Then the \textbf{determinant} of $f$ is defined as follows
        \begin{align*}
            \det:\mathrm{End}_{R\text{-}\mathsf{Mod}}\left(M\right) &\longrightarrow R\\
            f &\longmapsto \sum_{\sigma\in S_n}\mathrm{sgn}(\sigma)\prod_{i=1}^n a_{i,\sigma(i)}.
        \end{align*}
    \end{enumerate}
\end{definition}
